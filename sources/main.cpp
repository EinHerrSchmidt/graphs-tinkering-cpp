...
int main()
{
    ...
    Vertex<std::string> tf1(0, "A", "a");
    Vertex<std::string> tf2(3, "D", "d");
    Edge<std::string> etf(tf2, tf1);
    Edge<std::string> e = *udgManual.getEdges().find(etf);

    std::cout << e.getV1().getId() << " " << e.getV1().getLabel() << " " << e.getV1().getValue() << "\n";
    std::cout << e.getV2().getId() << " " << e.getV2().getLabel() << " " << e.getV2().getValue() << "\n";
